...
public class GameEngine {
    ...
    public void tick(double dt) {
        ...
        for (int i = vehicles.size() - 1; i >= 0; i--) {
            ...
            if (vehicle.hasReachedStop()) {
                RoadTile current = vehicle.getCurrentTile();
                RoadTile newGoal = vehicle.isGoingForward()
                        ? vehicle.getStopA()
                        : vehicle.getStopB();

                List<RoadTile> reversePath = pathFinder.findPath(current, newGoal);
                if (reversePath != null && reversePath.size() >= 2) {
                    vehicle.reversePath(reversePath);
                }
            }
        }
